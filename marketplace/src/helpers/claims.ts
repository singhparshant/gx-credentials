...
export const newDisplay = (ct: ClaimType): ClaimUIAssets => {
  switch (ct) {
    ...
    case 'github':
      return {
        description:
          'This process is used to link your GitHub account to your Tezos account by entering your GitHub handle, signing a message using your private key, and finally posting a public Gist containing that signature.',
...
export const getUnsignedAttestation = (
...
export const getFullAttestation = async (
  ...
  switch (subject.type) {
    case 'dns':
      return `${getUnsignedAttestation(subject)}=${await getSignedAttestation(
        subject,
        userData,
        wallet
      )}`;
