...
export const addClaims = async (claimsList: Array<helpers.Claim>) => {
  if (!localClient) {
    alert.set({
  ...
  try {
    ...
    await localClient.addClaims(did, claimsArgsList);
    ...
    w.client.subscribeToEvent(
      ...
      async (data) => {
        ...
        try {
          ...
          if (
            result?.message ===
            'Credentials have been published to the blockchain'
          ) {
